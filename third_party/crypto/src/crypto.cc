 ...
 */
    ...
    bool
    LogCrypt::GetLogHour(const char *const _data, size_t _len, int &_begin_hour, int &_end_hour) {
    ...
    void LogCrypt::UpdateLogTime(char *_data) {
    ...
    void LogCrypt::SetHeaderInfo(char *_data, bool _is_async) {
        ...
        uint32_t len = 0;
        memcpy(_data + sizeof(kMagicAsyncStart) + sizeof(seq_) + sizeof(hour) * 2, &len,
               sizeof(len));
    ...
    bool LogCrypt::GetPeriodLogs(const char *const _log_path, int _begin_hour, int _end_hour,
        ...
        while (!feof(file) && !ferror(file)) {
            ...
            if (!GetLogHour(header_buff, GetHeaderLen(), begin_hour, end_hour)) {
                snprintf(msg, sizeof(msg),
                         "__GetLogHour(buff.Ptr(), buff.UpdateLen(), beginHour, endHour) err, before_len:%ld.",
                         before_len);
                break;
            }
        ...
        if (!ret) {
            ...
            snprintf(msg, sizeof(msg), "begintpos:%lu, endpos:%lu, filesize:%ld.", _begin_pos,
                     _end_pos, file_size);
            _err_msg += msg;
        }
    ...
    void
    LogCrypt::CryptSyncLog(const char *const _log_data, size_t _input_len, AutoBuffer &_out_buff) {
    ...
    void
    LogCrypt::CryptAsyncLog(const char *const _log_data, size_t _input_len, AutoBuffer &_out_buff,
